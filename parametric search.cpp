...
void get_mos_cow(int *new_cow, int* cow, char *mos, char *temp_mos, int total_cow, int idx, int *count, int back_cow[][HELLCOW]) {
	...
	for (int s = 0; s < 3; s++) {
		temp_mos[idx] = mos[s];
		get_mos_cow(new_cow, cow, mos, temp_mos, total_cow, idx + 1, count, back_cow);
...
int main() {
	...
	int cow[HELLCOW];
	int new_cow[HELLCOW];
	int back_cow[HELLCOW + 5][HELLCOW];
	...
	int idx = 0;
	int count = 0;
	get_mos_cow(new_cow, cow, mos, temp_mos, cow_num, idx, &count, back_cow);
	cout << count;
}
